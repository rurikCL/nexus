        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 960, background: 'rgba(2,5,12,0.85)', display: 'grid', placeItems: 'center' }}
          ...
          <div className="nx-panel solid nx-panel-glow" style={{ padding: 18 }} onMouseDown={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
              {NX.CLASSES.map((c, i) => {
                const f = i + 1;
                const active = currentForma === f;
                /* Sin slots de habilidad = forma no aprendida: no seleccionable. */
                ...
                return (
                  <button key={c.id} disabled={!hasSlots && !active}
                    ...
                    style={{
                      ...
                      border: `1px solid ${active ? c.accent : hasSlots ? 'var(--holo-line)' : 'rgba(255,255,255,0.06)'}`,
                      background: active ? `color-mix(in srgb, ${c.accent} 14%, transparent)` : hasSlots ? 'rgba(255,255,255,0.02)' : 'rgba(255,255,255,0.01)',
                    }}>
                    <img src={c.img} alt="" style={{
                      width: 34, height: 34, objectFit: 'contain',
                      filter: active
                        ? `drop-shadow(0 0 6px ${c.accent})`
                        : hasSlots ? 'brightness(0.7) saturate(0.7)' : 'brightness(0.4) saturate(0.2)',
                    }} />
                    <span style={{ fontSize: 9, color: c.accent, fontFamily: 'var(--font-data)', opacity: hasSlots || active ? 1 : 0.4 }}>{c.num}</span>
                    {!hasSlots && !active && <span style={{ fontSize: 7, color: 'rgba(150,150,150,0.5)' }}>sin slots</span>}
                  </button>
                );
              })}
            </div>
